#!/usr/bin/env python3
"""
Diagnostic routine to check for real signal in layer 6 features
independent of Lasso's regularization.
"""
...
def load_layer6_data():
    """Load the SAE results and configuration for layer 6."""
    # Find the most recent experiment directory
    experiment_dirs = [d for d in os.listdir('experiment_logs') if d.startswith('experiment_')]
    if not experiment_dirs:
        raise FileNotFoundError("No experiment directories found")
    
    # Get the most recent one (assuming lexicographic order works)
    latest_dir = sorted(experiment_dirs)[-1]
    experiment_path = Path('experiment_logs') / latest_dir
    
    print(f"Loading data from: {experiment_path}")
    
    # Load SAE results
    sae_file = experiment_path / f"sae_results_layer_6.json"
...
def generate_task_labels(n_samples):
    """Generate synthetic task labels for demonstration."""
    # This is a placeholder - in a real scenario, you'd have actual task labels
    # For now, we'll create some synthetic labels that might correlate with features
    np.random.seed(42)
    return np.random.randn(n_samples)

def run_diagnostic():
    """Run the complete diagnostic routine."""
    print("🔍 Layer 6 Feature Signal Diagnostic")
    print("=" * 50)
    
    # === 1️⃣ Load SAE correlation data ===
    try:
        sae_output, config, experiment_path = load_layer6_data()
    ...
    plt.hist(abs_corrs, bins=50, color="gray", edgecolor="black", alpha=0.7)
    plt.title(f"Layer 6 Feature–Task Correlation Distribution")
    ...
    plt.axvline(np.median(abs_corrs), color='blue', linestyle='--', 
                label=f'Median: {np.median(abs_corrs):.3f}')
    plt.legend()
    
    # Save plot
    plot_path = experiment_path / "layer6_correlation_histogram.png"
    ...
    # Ridge coefficient statistics (if we had multiple features)
    if len(features_scaled) > 1:
        ridge_coefs = np.abs(ridge.coef_)
        print(f"\nRidge coefficient statistics:")
        print(f"Max |ridge_coef| = {ridge_coefs.max():.4f}")
        print(f"Mean |ridge_coef| = {ridge_coefs.mean():.4f}")
    ...
    results_path = experiment_path / "layer6_diagnostic_results.json"

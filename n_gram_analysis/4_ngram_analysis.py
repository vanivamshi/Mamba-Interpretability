...
"""
N-gram analysis of neuron activations with attention-aware detection
...
This script now includes attention-aware n-gram detection that addresses the "flat" 
appearance issue in Mamba models. The key insight is that neurons should only be 
considered n-gram detectors when the layer's implicit attention puts substantial 
mass on the last n tokens.

Configuration:
- USE_ATTENTION_AWARE_DETECTION: Enable/disable attention-aware detection
- ATTN_MASS_THRESHOLD: Minimum attention mass required (default: 0.1)
- EMPIREICAL_PROBE_SAMPLES: Number of texts for empirical probe (default: 5)

The system tries to use HiddenMambaAttn if available, otherwise falls back to 
an empirical finite-difference probe that estimates attention by measuring output 
changes when replacing tokens.
"""
...
# -------------------------
# Attention-aware n-gram detection helpers
# -------------------------

# Threshold for attention mass on last n tokens
ATTN_MASS_THRESHOLD = 0.1

# Configuration for attention-aware detection
USE_ATTENTION_AWARE_DETECTION = True  # Set to False to disable attention-aware detection
EMPIREICAL_PROBE_SAMPLES = 5  # Number of texts to use for empirical probe
...
def empirical_attention_probe(model, tokenizer, texts, layer_idx, n_tokens=3, device=None):
    """
    Empirical finite-difference probe to estimate attention mass on last n tokens.
    
    For each position, we replace the last n tokens with random tokens and measure
    the difference in layer output. Large differences indicate high attention mass.
    """
    if device is None:
    ...
    # Get a random token for replacement (avoid special tokens)
    vocab_size = tokenizer.vocab_size
    random_token_id = torch.randint(1, vocab_size, (1,)).item()  # Skip padding token
    
    def hook_fn(module, inp, out):
        if isinstance(out, tuple):
            out = out[0]
        acts = out.detach()
        attention_mass.append(acts.clone())
    
    handle = layers[layer_idx].register_forward_hook(hook_fn)
    
    try:
        with torch.no_grad():
            for text in texts[:EMPIREICAL_PROBE_SAMPLES]:  # Use subset for efficiency
                ...
                attention_mass = []
                _ = model(**inputs)
                original_output = attention_mass[0] if attention_mass else None
                ...
                # For each position, measure attention to last n tokens
                for pos in range(n_tokens, seq_len):
                    if attention_mask is not None and attention_mask[0, pos] == 0:
                        position_attention.append(0.0)
                        continue
                    
                    # Create modified input with last n tokens replaced
                    modified_ids = input_ids.clone()
                    for i in range(n_tokens):
                        if pos - i >= 0:
                            modified_ids[0, pos - i] = random_token_id
                    
                    # Forward pass with modified input
                    attention_mass = []
                    modified_inputs = {"input_ids": modified_ids}
                    if attention_mask is not None:
                        modified_inputs["attention_mask"] = attention_mask
                    _ = model(**modified_inputs)
                    modified_output = attention_mass[0] if attention_mass else None
                    
                    if modified_output is not None:
                        # Compute L2 norm of difference at this position
                        diff = torch.norm(original_output[0, pos] - modified_output[0, pos]).item()
                        position_attention.append(diff)
                    else:
                        position_attention.append(0.0)
                
                # Normalize attention mass (convert to probabilities)
                if position_attention:
                    max_attn = max(position_attention) if position_attention else 1.0
                    if max_attn > 0:
                        position_attention = [attn / max_attn for attn in position_attention]
                
                break  # Use only first text for efficiency
...
def get_attention_mass_for_layer(model, tokenizer, texts, layer_idx, n_tokens=3):
    """
    Get attention mass for last n tokens at each position.
    Uses MambaAttentionNeurons for Mamba models, falls back to empirical probe for others.
    """
    # Check if this is a Mamba model by looking for the attention_neurons module
    try:
        ...
        inputs = tokenizer(sample_text, return_tensors="pt", truncation=True, max_length=128, padding=True)
        ...
        if layer_idx in attention_data:
            ...
            if 'attention_matrix' in layer_data:
                ...
                print(f"✅ Extracted Mamba attention matrix: {attention_matrix.shape}")
            ...
            if attention_matrix is not None:
                # Extract attention mass on last n tokens for each position
                attention_mass = []
                seq_len = attention_matrix.shape[-1]  # Last dimension is sequence length
                
                for pos in range(seq_len):
                    if pos < n_tokens:
                        attention_mass.append(0.0)  # Not enough history
                    else:
                        # Sum attention mass on last n tokens
                        # attention_matrix shape: [batch, heads, seq, seq]
                        # We want attention from position pos to positions [pos-n_tokens+1, pos]
                        start_pos = max(0, pos - n_tokens + 1)
                        mass = attention_matrix[0, :, pos, start_pos:pos+1].sum().item()  # Sum across heads and last n positions
                        attention_mass.append(mass)
                
                return attention_mass
            else:
                print(f"⚠️  No attention matrix found for layer {layer_idx}, falling back to empirical probe")
                
    except Exception as e:
        print(f"⚠️  MambaAttentionNeurons failed: {e}, falling back to empirical probe")
    
    # Fall back to empirical finite-difference probe
    print(f"Using empirical attention probe for layer {layer_idx}")
    return empirical_attention_probe(model, tokenizer, texts, layer_idx, n_tokens)

# -------------------------
# N-gram trigger collection
# -------------------------
def collect_ngram_triggers(model, tokenizer, texts, layer_idx, n_max=3, model_label=None):
    """Collect n-gram triggers for neurons up to n_max with attention-aware detection."""
    ...
    # Get attention mass for this layer (for attention-aware detection)
    attention_mass_per_n = {}
    if USE_ATTENTION_AWARE_DETECTION:
        print(f"Computing attention mass for layer {layer_idx}...")
        for n in range(1, n_max + 1):
            attention_mass_per_n[n] = get_attention_mass_for_layer(model, tokenizer, texts[:5], layer_idx, n)
    else:
        print(f"Using traditional n-gram detection (attention-aware disabled)")
        # Initialize with dummy attention mass (all positions have high attention)
        for n in range(1, n_max + 1):
            attention_mass_per_n[n] = [1.0] * 128  # Assume max sequence length
    ...
    def hook_fn(module, inp, out):
        ...
        for b in range(acts.shape[0]):
            ...
            for t in range(seq_len):
                ...
                # For unigrams (n=1), check attention mass on current token
                if t < len(attention_mass_per_n[1]):
                    attn_mass_1 = attention_mass_per_n[1][t]
                    if attn_mass_1 >= ATTN_MASS_THRESHOLD:
                        for n in active_neurons:
                            neuron_triggers[int(n)][1].add((tok_id,))
                
                # For n-grams (n>1), check attention mass on last n tokens
                for nsize in range(2, n_max + 1):
                    if t + nsize <= seq_len and all(mask[t+i] == 1 for i in range(nsize)):
                        # Check if layer attends to the last nsize tokens at position t
                        if t < len(attention_mass_per_n[nsize]):
                            attn_mass_n = attention_mass_per_n[nsize][t]
                            if attn_mass_n >= ATTN_MASS_THRESHOLD:

...
def _find_dead_neurons_core(model, tokenizer, texts, layer_idx, threshold, freq_cutoff=0.001):
    """
    Core logic for finding dead neurons.
    - Active if activation > threshold
    - Dead if active in <0.1% of tokens (freq_cutoff).
    """
    model.eval()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    ...
    activation_freq = activation_counts.cpu().numpy() / max(total_tokens, 1)
    dead_neurons = [i for i, freq in enumerate(activation_freq) if freq < freq_cutoff]

    print(f"Found {len(dead_neurons)} dead neurons ({len(dead_neurons)/hidden_size:.1%} of layer)")
    print(f"Using threshold: {threshold}, frequency cutoff: {freq_cutoff}")
...
def find_dead_neurons(model, tokenizer, texts, layer_idx=0, freq_cutoff=0.001):
    """
    Dead neurons per paper definition, using a default threshold of 0.7.
    """
    return _find_dead_neurons_core(model, tokenizer, texts, layer_idx, threshold=0.7, freq_cutoff=freq_cutoff)
...
def run_complete_neuron_analysis(model, tokenizer, texts, layer_idx=0, delta_neurons=None, pruning_ratio=0.1):
    ...
    This function will continue to use the default find_dead_neurons (threshold=0.7).
    """
    ...
    # Step 2: Dead neurons (uses the default threshold of 0.7)

...
def extract_deltas_fixed(model, layer_idx, input_ids):
    ...
    def delta_hook(module, input, output):
        ...
        if hidden_states is not None:
            ...
            delta_values.append(torch.randn(batch_size, seq_len, hidden_size))
...
def find_delta_sensitive_neurons_fixed(model, tokenizer, texts, layer_idx=0, top_k=10):
    ...
    for i, text in enumerate(texts):
        try:
            print(f"Processing text {i+1}/{len(texts)}: {text[:300]}...")
            ...
            if delta_mean.shape[0] != hidden_size:
                ...
                if delta_mean.shape[0] < hidden_size:
                    padding = torch.zeros(hidden_size - delta_mean.shape[0])
            ...
            print(f"Error processing text '{text[:300]}...': {e}")
...
def perturb_neurons(tensor, neuron_indices, mode="zero", std=1.0):
    ...
    for idx in valid_indices:
        if mode == "zero":
            ...
            perturbed[..., idx] += torch.randn_like(perturbed[..., idx]) * std
        elif mode == "mean":

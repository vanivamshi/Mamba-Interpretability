...
# -------------------------
def collect_ngram_triggers(model, tokenizer, texts, layer_idx, n_max=3, activation_threshold=1.0):
...
def plot_paper_figure2(all_layer_triggers, all_dead_neurons, model_label, save_dir="plots"):
    ...
    depths = [l/num_layers for l in range(num_layers//2)]  # first half only
...
if __name__=="__main__":
    ...
    for model_label,model_name in models_to_analyze.items():
        ...
        for l in range(num_layers):
            ...
            dead,_=find_dead_neurons(model,tokenizer,texts[:50],l)
            trig=collect_ngram_triggers(model,tokenizer,texts[:100],l)

...
def plot_dead_neuron_distribution(all_models_data, save_path=None):
    ...
    Args:
        all_models_data (dict): Dictionary where keys are model names and values are
                                dicts mapping layer indices to dead neuron ratios.
    ...
    for model_label, dead_neuron_data in all_models_data.items():
        layers = list(dead_neuron_data.keys())
        dead_ratios = [dead_neuron_data[layer] for layer in layers]

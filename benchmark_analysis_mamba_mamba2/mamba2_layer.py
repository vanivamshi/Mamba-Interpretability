...
# ============ SIMPLIFIED FIXED MAMBA2 LAYER ============ #
class FixedMamba2Layer(nn.Module):
    """
    Simplified Mamba2 layer with reduced complexity for better stability.
    Features:
...
def create_simplified_mamba2(d_model, d_state=16):
    """
    Minimal, verified Mamba2 implementation.
    Creates a simplified sequential Mamba2 module.
    ...
    Returns:
        Sequential module with Mamba2 components
    """
...
def initialize_from_mamba(enhanced_model, original_mamba):
    """
    Copy weights from working Mamba to Mamba2 where possible.
    This helps initialize Mamba2 with pre-trained Mamba weights.
    
    Args:
        enhanced_model: The Mamba2 model to initialize
    ...
    logger.info(f"Copied {copied_count}/{len(enhanced_dict)} parameters from Mamba to Mamba2")
...
def attach_mamba2_layers(model: nn.Module, attribute_name: str = "mamba2", use_fixed=True) -> int:
    """
    FIXED: Attach Mamba2 layers with proper integration.
    
    Args:
        model: The model to attach Mamba2 layers to
        attribute_name: Name of the attribute to store Mamba2 layer
        use_fixed: If True, use simplified FixedMamba2Layer (recommended)
    """
    if use_fixed:
        logger.info("Attaching simplified FixedMamba2 layers...")
    else:
    ...
    for idx, layer in enumerate(layers):
        ...
        # Use FixedMamba2Layer if requested, otherwise use original complex version
        if use_fixed:
            # Simplified FixedMamba2Layer
            mamba2_layer = FixedMamba2Layer(d_model=d_model, n_gates=2, d_state=16)
        else:
            # Original complex Mamba2Layer (fallback)
            try:
            ...
            # Fallback to simple Mamba2Layer
...
# ============ TEST FIXED MAMBA2 LAYER ============ #
def test_fixed_mamba2_layer():
    """
    Test the simplified FixedMamba2Layer with gradient flow and forward pass verification.
    """
    print("="*60)
    print("Testing FixedMamba2Layer")
    print("="*60)
    ...
    layer = FixedMamba2Layer(d_model=d_model, n_gates=2, d_state=16)
    print(f"✓ Created FixedMamba2Layer with d_model={d_model}, d_state=16")
    ...
    print("\n" + "="*60)
    print("✓ All tests passed for FixedMamba2Layer!")
    print("="*60)
    return True
...
if __name__ == "__main__":
    # Test FixedMamba2Layer
    print("\n=== Testing FixedMamba2Layer ===\n")
    test_fixed_mamba2_layer()
    
    print("\n=== Testing original Mamba2Layer ===\n")

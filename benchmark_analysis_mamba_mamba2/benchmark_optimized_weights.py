        ...
        class Mamba2ActiveLayer(nn.Module):
            ...
            def forward(self, hidden_states, past_key_value=None, cache_position=None,
                ...
                try:
                    ...
                    combined = (gpt2_hidden * (1 - self.mamba2_weight) + 
                               mamba2_out * self.mamba2_weight)
...
def load_mamba2(mamba2_weight: float = 0.15, model_name: str = "Mamba2"):
    """Load Mamba2 with CLEAN mixing (no temperature/confidence modifications)"""
    print(f"Loading {model_name} (GPT-2 + Mamba2 @ {mamba2_weight:.0%})...")
...
def create_niah_prompt_better(context, needle_key):
    """
    Improved prompt - simpler and more direct
    Based on how Mamba successfully extracts numbers (it generated the number directly)
    """
    ...
    for i in range(n_samples):
        try:
            if task_type == "NIAH":
                ...
                # BETTER PROMPT - references "magic number" pattern
                prompt = create_niah_prompt_better(context, needle_key)
                ...
                if model_name and "GPT2" in model_name and "Mamba2" not in model_name:
                ...
                outputs = model.generate(**inputs, max_new_tokens=30, do_sample=False, pad_token_id=pad_token_id)
                outputs = torch.clamp(outputs, 0, vocab_size - 1)

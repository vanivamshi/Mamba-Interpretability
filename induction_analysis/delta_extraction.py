...
def extract_deltas_fixed(model, layer_idx, input_ids):
    ...
    if layers is None:
        ...
        return torch.randn(batch_size, seq_len, 512, device=device)  # Assuming 512 hidden size
    
    if layer_idx >= len(layers):
        print(f"Layer {layer_idx} not found. Using dummy values.")
        batch_size, seq_len = input_ids.shape
        return torch.randn(batch_size, seq_len, 512, device=device)
    ...
    def delta_hook(module, input, output):
        if isinstance(output, tuple):
            ...
            delta_values.append(output[0].detach() if len(output) > 0 else torch.randn(1, 1, 512, device=device))
        else:

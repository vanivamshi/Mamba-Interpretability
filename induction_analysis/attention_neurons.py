...
class MambaAttentionNeurons:
    ...
    def visualize_neurons(self, mamba_neurons: dict, layer_idx: int = 0, save_path: Optional[str] = None):
        ...
        fig.suptitle(f'Mamba Neurons Analysis - Layer {layer_idx}', fontsize=14)
        
        # Plot 1: Neuron activations
        if 'neuron_activations' in neurons:
            activations = neurons['neuron_activations'].cpu().numpy()
            axes[0, 0].bar(range(len(activations)), activations)
            axes[0, 0].set_title('Neuron Activations', fontsize=14)
            axes[0, 0].set_xlabel('Neuron Index', fontsize=14)
            axes[0, 0].set_ylabel('Activation Value', fontsize=14)
        ...
        if 'neuron_importance' in neurons:
            ...
            axes[0, 1].set_title('Neuron Importance', fontsize=14)
            axes[0, 1].set_xlabel('Neuron Index', fontsize=14)
            axes[0, 1].set_ylabel('Importance Score', fontsize=14)
        ...
        if 'attention_vectors' in neurons:
            ...
            axes[1, 0].set_title('Attention Heatmap', fontsize=14)
            axes[1, 0].set_xlabel('Sequence Position', fontsize=14)
            axes[1, 0].set_ylabel('Neuron Index', fontsize=14)
        ...
        if 'neuron_activations' in neurons:
            ...
            axes[1, 1].set_title('Top 10 Neurons', fontsize=14)
            axes[1, 1].set_xlabel('Neuron Rank', fontsize=14)
            axes[1, 1].set_ylabel('Activation Value', fontsize=14)
            axes[1, 1].set_xticks(range(len(top_indices)))
            axes[1, 1].set_xticklabels([f'#{idx}' for idx in top_indices], fontsize=14)

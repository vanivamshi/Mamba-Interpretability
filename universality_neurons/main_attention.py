...
delta_mamba = find_delta_sensitive_neurons_fixed(
    ...
    texts[:10],  # can adjust as desired
    layer_idx=0, top_k=10
)

print("\nAnalyzing variance of hidden neurons in Transformer...")
def transformer_neuron_variance(model, tokenizer, texts, layer_idx=0, top_k=10):
    activations = []
    for text in texts:
...
indices = np.arange(len(mamba_vars))
plt.bar(indices - 0.2, mamba_vars, width=0.4, label="Mamba Δ Neurons", color='lightgreen')
plt.bar(indices + 0.2, transformer_vars, width=0.4, label="Transformer Neurons", color='orange')

plt.xticks(indices, [str(i) for i, _ in delta_mamba])
plt.xlabel("Neuron Index")
plt.ylabel("Variance")
plt.title("Delta-Sensitive Neurons (Mamba) vs Variance Neurons (Transformer)")
plt.legend()
plt.tight_layout()

...
class MambaAttentionNeurons:
    ...
    def _create_attention_weighted_neurons(self, layer_data: dict):
        ...
        min_val = attention_vectors.min()
        max_val = attention_vectors.max()
        # Avoid division by zero
        if max_val > min_val:
            normalized_attention = (attention_vectors - min_val) / (max_val - min_val)
        else:
            normalized_attention = attention_vectors * 0.0  # All zeros if min == max
    ...
    def _create_rollout_neurons(self, layer_data: dict):
        ...
        if 'attention_matrix' not in layer_data:
            return None
        
        # Use the full attention matrix for rollout, not the averaged vectors
        ...
        eye = torch.eye(seq_len).expand(batch_size, num_heads, seq_len, seq_len).to(attention_matrix.device)

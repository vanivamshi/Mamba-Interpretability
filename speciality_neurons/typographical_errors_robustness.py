...
def count_activated_neurons(activation_vector, threshold=0.1):
    """
    Count the number of neurons that are activated above a threshold.
    
    Args:
        activation_vector: numpy array of neuron activations
        threshold: activation threshold (default: 0.1)
        
    Returns:
        Number of activated neurons
    """
    return np.sum(np.abs(activation_vector) > threshold)
...
def test_typo_robustness(model, tokenizer, texts, specialty_neurons=None, layer_idx=0, 
    ...
    results = {
        ...
        'neuron_sensitivity': {},
        'typo_examples': {},
        'activation_counts': {}  # New field for tracking neuron counts
    }
    ...
    for typo_type in typo_types:
        ...
        results['activation_counts'][typo_type] = []  # Track activation counts per typo type
        ...
        try:
            ...
            for typo_type in typo_types:
                ...
                for variant in range(num_variants):
                    ...
                    activation_count = count_activated_neurons(typo_activation)
                    typo_activation_counts.append(activation_count)
                
                # Store results
                results['typo_activations'][typo_type].append(typo_activations)
                results['typo_examples'][typo_type].append(typo_texts)
                results['activation_counts'][typo_type].append(typo_activation_counts)
            ...
            for typo_type in typo_types:
                ...
                results['activation_counts'][typo_type].append([len(dummy_activation)] * num_variants)  # All neurons activated
            ...
            for typo_type in typo_types:
                ...
                for neuron_idx, _ in neurons:
                    ...
                    for text_idx in range(len(texts)):
                        if (text_idx < len(results['consistency_scores'][typo_type]) and 
                            text_idx < len(results['typo_activations'][typo_type])):
                            
                            # Get activations for this specific neuron
                            original_activation = results['original_activations'][text_idx]
                            typo_activations = results['typo_activations'][typo_type][text_idx]
                            
                            if (len(original_activation) > neuron_idx and 
                                all(len(typo_act) > neuron_idx for typo_act in typo_activations)):
                                
                                for typo_activation in typo_activations:
                                    # Calculate consistency for this specific neuron
                                    neuron_original = original_activation[neuron_idx]
                                    neuron_typo = typo_activation[neuron_idx]
                                    
                                    # Simple consistency: how much the neuron activation changes
                                    if neuron_original != 0:
                                        consistency = 1.0 - min(1.0, abs(neuron_typo - neuron_original) / abs(neuron_original))
                                    else:
                                        consistency = 1.0 if neuron_typo == 0 else 0.0
...
def analyze_typo_robustness_results(results):
    ...
    if 'activation_counts' in results:
        print("Neuron Activation Counts:")
        print("-" * 40)
        
        for typo_type in typo_types:
            if typo_type in results['activation_counts']:
                all_counts = []
                for text_counts in results['activation_counts'][typo_type]:
                    all_counts.extend(text_counts)
                ...
                if all_counts:
                    ...
                    print(f"{typo_type.upper()} Typos:")
                    print(f"  Mean activated neurons: {mean_count:.1f}")
                    print(f"  Std deviation:         {std_count:.1f}")
                    print(f"  Min activated:         {min_count}")
                    print(f"  Max activated:         {max_count}")
                    print()
